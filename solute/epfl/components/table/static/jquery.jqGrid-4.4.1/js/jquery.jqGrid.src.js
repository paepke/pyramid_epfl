...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		sortData = function (index, idxcol,reload,sor){
            ...
            if (('multisort' in ts.p) && (ts.p.multisort)){
                if (typeof ts.p.sortname === 'string') {
                    ts.p.sortname = {};
                }
                var name = ts.p.colModel[idxcol].index || index;
                if (name in ts.p.sortname){
                    ts.p.sortname[name][0] = ts.p.sortorder;
                } else {
                    ts.p.sortname[name] = [ts.p.sortorder, Object.keys(ts.p.sortname).length];
                }
                if (ts.p.sortname){
                    $(ts.p.reset_sort).show();
                }
                setSortOrder();
            }else{
        ...
        setSortOrder = function () {
            for (var i=0; i < ts.grid.headers.length; i++){
                if (ts.p.colModel[i].sortable){
                    var current = ts.grid.headers[i].el;
                    var col_name = ts.p.colModel[i].name;
                    if (col_name in ts.p.sortname){
                        $("sup.col-sort-num",current).html(ts.p.sortname[col_name][1]).show();
                        $("sup.ui-jqgrid-reset-button", current).show();
                    } else {
                        $("sup.col-sort-num",current).html('').hide();
                        $("sup.ui-jqgrid-reset-button", current).hide();
                    }
                }
            }
        },
		...
		for(i=0;i<this.p.colNames.length;i++){
            ...
            if (('multisort' in ts.p) && (ts.p.multisort)){
                thead += imgs + "<sup class=\"col-sort-num\" style=\"margin-left: 15px; font-size: 9px;\"></sup><sup rsname=\"" + ts.p.colModel[i].index + "\" class=\"ui-jqgrid-reset-button\" style=\"display: none\"></sup></div></th>";
            }
            else {
                thead += imgs + "</div></th>";
            }
		...
		imgs = null;
		$(this).append(thead);
        $("sup.ui-jqgrid-reset-button").click(function(){
            var default_id = 0;
            var name = $(this).attr("rsname");
            var pos = NaN;
            if (name in ts.p.sortname){
                pos = ts.p.sortname[name][1];
            }
            if (pos != NaN){
                var buff = {};
                for (var key in ts.p.sortname) {
                    if (key == name){
                        continue;
                    }
                    else if (ts.p.sortname[key][1] > pos){
                        buff[key] = [ts.p.sortname[key][0], ts.p.sortname[key][1] - 1];
                    }
                    else{
                        buff[key] = ts.p.sortname[key];
                    }
                }
                ts.p.sortname = buff;
                if (ts.p.sortname == {}){
                    if (ts.p.sortname_default != 'None'){
                        for (var i=0; i < ts.p.colModel.length; i++){
                            if (ts.p.colModel[i].name == ts.p.sortname_default){
                                default_id = i;
                                ts.p.sortname[ts.p.sortname_default] = [ts.p.sortorder_default, 0];
                                break;
                            }
                        }
                    }
                    ts.p.sortorder = ts.p.sortorder_default;
                }
                var last = ts.grid.headers[ts.p.lastsort].el;
                $("div span.s-ico",last).hide();
                $("div span.ui-icon-"+ts.p.sortorder,last).addClass("ui-state-disabled");
                ts.p.lastsort = default_id;
                ts.p.addSort = false;
                setSortOrder();
                populate();
            }
