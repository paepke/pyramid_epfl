$(document).ready(function () {
    ...
    epfl.TableComponent = function (cid, params) {
        ...
        params["opts"]["onHeaderClick"] = function (gridstate) {
            compo_obj.on_header_click.call(compo_obj, gridstate)
        };

        params["opts"]["datatype"] = function (postdata) {
            compo_obj.data_type_func.call(compo_obj, postdata);
        };

        // resize column handling
        params["opts"]["resizeStop"] = function (new_width, col_idx) {
            compo_obj.resize_top_func.call(compo_obj, new_width, col_idx)
        };

        if (!params["opts"]["pgtext"]) {
            delete params["opts"]["pgtext"]
        }
        ;
        ...
        this.table_container_div.scroll(function () {
            compo_obj.table_container_div_scrollfn.call(compo_obj)
        });
    ...
    epfl.TableComponent.prototype.table_container_div_scrollfn = function () {
        ...
        var ev = this.make_event("setScrollPos", {
            "top": top_pos,
            "left": left_pos
        });
    ...
    epfl.TableComponent.prototype.resize_stop_func = function (new_width, col_idx) {
        var ev = this.make_event("setColumnWidth", {
            "col_idx": col_idx,
            "new_width": new_width
        });
        epfl.enqueue(ev);
...
// formatters
    jQuery.extend($.fn.fmatter, {
        "icon_formatter": function (cellvalue, options, row) {
            ...
            if (cellvalue["tip"]) {
                alt_attr = "alt='" + cellvalue["tip"] + "' title='" + cellvalue["tip"] + "'";
            }
            if (cellvalue["link"]) {
            ...
            if (cellvalue["cmd"]) {
                lnk_open = "<div class='epfl_clickevent clickable' cmd='" + cellvalue["cmd"] + "'>";
                lnk_close = "</div>"
            }
